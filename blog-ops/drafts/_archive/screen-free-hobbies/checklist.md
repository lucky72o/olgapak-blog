...
- **Live inbound-link pass (Gate 2 finalize, console-merge path, 2026-09-02).** WP post 2215 was found `status: publish` (human published in wp-admin before finalize ran), and `publish.wordpress.apply_inbound_links_live: true`, so the three planned rows are applied to the LIVE WordPress posts via REST. Write-ahead records, written BEFORE editing; each target was fetched with `context=edit` and verified to contain zero `/screen-free-hobbies` links first:
  - live inbound link planned: how-to-reduce-screen-time (wp id 2181) -> this post
  - live inbound link planned: digital-detox-plan (wp id 2152) -> this post
  - live inbound link planned: how-to-stop-doomscrolling (wp id 2122) -> this post
  Anchors use the root-relative `/screen-free-hobbies` form rather than the adapter's absolute `$WP_BASE/screen-free-hobbies`: every existing internal link in all three live posts is root-relative, the rendered result is identical, and the adapter's own idempotency pattern matches both forms.
  Results (all HTTP 200, each verified back via `context=edit` and on the rendered live page):
  - live inbound link applied: how-to-reduce-screen-time (wp id 2181) -> this post, anchor "screen-free hobbies worth sticking with" in the "Replace, don't just remove" bullet; before: `...things to do instead of reaching for your phone</a> that don't require a new hobby or any equipment.</p>`; after: `...any equipment, and if you'd rather the hour went somewhere that compounds, there are <a href="/screen-free-hobbies">screen-free hobbies worth sticking with</a> sorted by what each one actually costs to start.</p>`
  - live inbound link applied: digital-detox-plan (wp id 2152) -> this post, anchor "screen-free hobbies to fill the gap" in the closing "where the reclaimed time goes" paragraph; before: `...<a href="/how-to-plan-your-week">planning your week</a> than another hour on settings.</p>`; after: `...than another hour on settings. If the week after the reset feels emptier than you expected, that's the gap to fill on purpose, and there are <a href="/screen-free-hobbies">screen-free hobbies to fill the gap</a> with an honest note on what each costs.</p>`
  - live inbound link applied: how-to-stop-doomscrolling (wp id 2122) -> this post, anchor "a hobby to put in its place" in "7. Replace, don't just remove"; before: `...things to do instead of being on your phone</a>.</p>`; after: `...things to do instead of being on your phone</a>, and if the replacement needs to be bigger than a filler activity, pick <a href="/screen-free-hobbies">a hobby to put in its place</a> instead.</p>`
- **Gate 2 finalize (2026-09-02).** Console-merge path: PR #20 merged on origin at 2026-09-02T19:01:05Z, `approval.json` present (operator, mode `now`, 19:00:59Z). No monitor cron existed under console-gated mode, so the `CronDelete` step was a no-op. WP post 2215 was already `status: publish` (live at https://olgapak.com/screen-free-hobbies), so the final content re-sync was SKIPPED per the adapter's external-publish rule; the live post is authoritative and none of its in-admin edits were overwritten. The branch archive snapshot was already squash-merged to `main` via PR #20; this finalize only brings it to terminal state (`status: complete`, `pr-monitor.json` `status: done`) and adds the live inbound-link record above.
